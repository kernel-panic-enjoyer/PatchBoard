...
A single-binary Go Windows updater with a browser UI for winget and Chocolatey.

## Features

- Runs as a local-only WebUI on `127.0.0.1`.
- Relaunches itself as administrator when needed.
- Detects winget and Chocolatey.
- Lists installed winget and Chocolatey packages in one table.
...
- Installs packages from winget or Chocolatey after an explicit button click.
...
- Scans Windows uninstall registry plus winget inventory and reports apps newly detected since the previous scan.
