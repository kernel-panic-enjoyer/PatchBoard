...
func TestRenderedHTMLContainsAsyncUpdateHooks(t *testing.T) {
	...
	for _, expected := range []string{
		...
		`data-tooltip="Every retained session log entry from PatchBoard and package-manager processes."`,
		`aria-label="All. Every retained session log entry from PatchBoard and package-manager processes."`,
		`.log-tab:hover::after`,
		`.log-tab:focus-visible::after`,
		`id="select-all-updates"`,
		`id="select-no-updates"`,
